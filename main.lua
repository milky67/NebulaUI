...
-- CONFIG (Baguhin mo lang dito)
local Config = {
    Key = "NEBULA2025-FREE",           -- Palitan mo ng sarili mong key
    Discord = "https://discord.gg/nebula2025",  -- Discord link mo
...
-- Notification Function (Gumagana sa mobile)
function NebulaUI:Notify(Title, Text, Duration)
    ...
    spawn(function()
        ...
        task.wait(Duration)
        TweenService:Create(Notif, TweenInfo.new(0.5), {Position = UDim2.new(1, 20, 1, -110)}):Play()
        task.wait(0.6)
        Notif:Destroy()
    end)
end
...
function NebulaUI:Init()
    ...
    TitleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            local delta = input.Position - Main.Position
            local conn
            conn = RunService.RenderStepped:Connect(function()
                if not dragging then conn:Disconnect() return end
                Main.Position = UDim2.new(0, input.Position.X - delta.X, 0, input.Position.Y - delta.Y)
            end)
    ...
    function NebulaUI:CreateTab(Name)
        ...
        function Tab:Dropdown(Text, Options, Callback)
            ...
            Arrow.MouseButton1Click:Connect(function()
                open = not open
                List.Visible = open
                TweenService:Create(Drop, TweenInfo.new(0.3), {Size = open and UDim2.new(0,440,0,55+#Options*45) or UDim2.new(0,440,0,55)}):Play()
            end)
    ...
    NebulaUI:Notify("Welcome!", "NebulaUI v3 Loaded Successfully!", 5)
end
